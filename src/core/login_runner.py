...
from src.auth.login import wait_for_waiting_room, login, CloudflareBlockException
from src.auth.security import handle_security_question
from src.utils.common import human_delay
...
def _get_args() -> argparse.Namespace:
    ...
    if not _ARGS.profile_dir:
        _ARGS.profile_dir = str(Path(__file__).parent.parent.parent / f"chrome_profile_{_ARGS.customer}")
    return _ARGS
...
async def run() -> None:
    ...
    if not FASTCAPTCHA_API_KEY:
        log.info("FASTCAPTCHA_API_KEY missing; CAPTCHA will be handled manually in the browser if it appears.")
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            log.error(f"Cloudflare Block detected: {e}. Exiting with code 43 to trigger session wipe and retry.")
            try:
                await page.screenshot(path=f"error_{args.customer}_cf_block.png")
            except Exception:
                pass
            sys.exit(43)
